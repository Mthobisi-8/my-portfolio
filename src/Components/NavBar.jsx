    ...
    <div className="flex justify-between items-center w-full h-15 px-4 text-white bg-black fixed  z-20 backdrop-blur-md">
      ...
      <div>
        <img
                    src={HeroImage}
                    alt="my profile"
                    className=" lg:w-25 lg:h-13 hidden md:flex md:w-30  ml-0 "
                  />
      </div>
      ...
      <ul className="hidden md:flex backdrop-blur-sm z-20">
        {
          links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize font-medium text-gray-500 hover:scale-105 duration-200 backdrop-blur-sm z-50">
